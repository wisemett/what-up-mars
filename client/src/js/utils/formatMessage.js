...
const getGap = (time) => {
  ...
  else if (now.diff(then, 'months') > 0)
    return [now.diff(then, 'months'), 'month'];
  else if (now.diff(then, 'days') > 0) return [now.diff(then, 'days'), 'day'];
  else if (now.diff(then, 'hours') > 0)
    return [now.diff(then, 'hours'), 'hour'];
  else if (now.diff(then, 'minutes') > 0)
    return [now.diff(then, 'minutes'), 'minute'];
